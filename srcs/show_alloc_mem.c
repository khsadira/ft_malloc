...
static long long int	print_all_block(t_block *block)
{
...
static long long int 	show_tiny_mem()
{
	...
	if (g_page.tiny_region != NULL)
	{
		...
		total_size = print_all_block(g_page.tiny_region->head_block);
	}
	return total_size;
}
...
static long long int	show_small_mem()
{
	...
	if (g_page.small_region != NULL)
	{
		...
		total_size = print_all_block(g_page.small_region->head_block);
...
static long long int	show_large_mem()
{
	...
	if (g_page.heavy_region != NULL)
	{
		...
		total_size = print_all_block(g_page.heavy_region->head_block);
	}
	return total_size;
}
